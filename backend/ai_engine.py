...
def execute_ai_agent(agent_name: str, system_prompt: str, user_input: str) -> str:
    """
    Executes an AI Agent task using OpenAI API or Gemini API fallback.
    Returns intelligent, detailed, structured responses tailored to the specific agent domain.
    """
    openai_api_key = os.getenv("OPENAI_API_KEY")
    gemini_api_key = os.getenv("GEMINI_API_KEY")

    prompt = f"{system_prompt}\n\nUSER PAYLOAD / INPUT:\n{user_input if user_input else 'Process sample query and generate comprehensive analysis.'}"
    ...
    # 3. Domain-specific high quality AI response generator fallback
